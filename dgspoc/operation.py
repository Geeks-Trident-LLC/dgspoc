...
def get_test_script_files(directory, case=''):
    ...
    if case:
        lookup = '# %s script is generated by Describe-Get-System' % case
    else:
        lookup = ('# (?P<framework>unitest|pytest|robotframework) script '
                  'is generated by Describe-Get-System')
...
def do_build_batch_script(options):
    ...
    if not len(operands):
        for case in tbl:
            result = get_test_script_files('.', case=case)
            dst_dir = 'test_script_files/%(case)' % result
            copied_files = File.copy_files(result.files, dst_dir)
            tbl[case] = copied_files
    else:
        for item in operands:
            ...
            if item.lower() in tbl:
                ...
                dst_dir = 'test_script_files/%(case)' % result
                copied_files = File.copy_files(result.files, dst_dir)
                tbl[case] = list(set(tbl[case] + copied_files))
            else:
                pat = '(?i)(?P<case>unittest|pytest|robotframework)='
                match = re.match(pat, item)
                if match:
                    directory = re.sub(pat, '', item)
                    case = match.group('case').lower()
                    result = get_test_script_files(directory, case=case)
                    dst_dir = 'test_script_files/%(case)' % result
                    copied_files = File.copy_files(result.files, dst_dir)
                    tbl[case] = list(set(tbl[case] + copied_files))
                else:
                    directory = item
                    result = get_test_script_files(directory)
                    if result.case:
                        dst_dir = 'test_script_files/%(case)' % result
                        copied_files = File.copy_files(result.files, dst_dir)
                        tbl[case] = list(set(tbl[case] + copied_files))
    ...
    dt = datetime.now()
    unittest_report_fn = File.get_filepath_timestamp_format1(
        prefix='unittest_report', extension='xml', ref_datetime=dt
    )
    pytest_report_fn = File.get_filepath_timestamp_format1(
        prefix='pytest_report', extension='xml', ref_datetime=dt
    )
    robot_log_fn = File.get_filepath_timestamp_format1(
        prefix='robotframework_log', extension='html', ref_datetime=dt
    )
    robot_output_fn = File.get_filepath_timestamp_format1(
        prefix='robotframework_output', extension='xml', ref_datetime=dt
    )
    robot_report_fn = File.get_filepath_timestamp_format1(
        prefix='robotframework_report', extension='html', ref_datetime=dt
    )
    ...
    robot_fmt = 'python -m robot --log=%s --output=%s --report=%s test_script_files/robotframework'

    lst = []
    for fw, files in tbl.items():
        if files:
            if fw == FWTYPE.UNITTEST:
                console_cmdline = unittest_fmt % unittest_report_fn
                console_cmdline not in lst and lst.append(console_cmdline)
            elif fw == FWTYPE.PYTEST:
                console_cmdline = pyttest_fmt % pytest_report_fn
                console_cmdline not in lst and lst.append(console_cmdline)
            else:
                group = tuple(robot_log_fn, robot_output_fn, robot_report_fn)
                console_cmdline = robot_fmt % group
                console_cmdline not in lst and lst.append(console_cmdline)
